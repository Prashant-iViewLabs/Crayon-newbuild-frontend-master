...
export default function ButtonPanel({ panelData, side, topMargin = false, onChangeFilter = () => { } }) {
    ...
    const removeBtnSelection = (selectedBtnList, id) => {
        selectedBtnList.splice(selectedBtns.findIndex(selectedBtn => selectedBtn == id), 1)
    }
    const handleButtonClick = (event, { id, name, title }) => {
        if (title == true) {
            ...
            if (selectedBtns.find(selectedBtn => selectedBtn == id)) {
                ...
                if (selectedBtns.find(selectedBtn => selectedBtn == panelData[0]?.id)) {
                    const selectedBtnList = [...selectedBtns]
                    removeBtnSelection(selectedBtnList, id)
            ...
            {panelData?.map(btn => (
                ...
                <Button
                    sx={{
                        ...
                        fontWeight: (btn.title || selectedBtns.includes(btn.id)) ? 900 : 400,
                        '&:hover': {
                            boxShadow: 15,
                            // opacity: 1,
